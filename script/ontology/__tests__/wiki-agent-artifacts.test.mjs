...
test("agent artifacts expose compact query and node payloads", () => {
	const artifacts = buildWikiAgentArtifacts({
		...
		relations: [
			{
				id: "relation:doc:ceph-osd:about_subject:subject:infrastructure:storage-system:ceph",
				from: "doc:ceph-osd",
	...
	assert.deepEqual(artifacts.nodes["subject:infrastructure:storage-system:ceph"].relations, [
		{
			id: "relation:doc:ceph-osd:about_subject:subject:infrastructure:storage-system:ceph",
			predicate: "about_subject",
	...
	assert.deepEqual(artifacts.nodes["doc:ceph-osd"].relations, [
		{
			id: "relation:doc:ceph-osd:about_subject:subject:infrastructure:storage-system:ceph",
			predicate: "about_subject",
			other_id: "subject:infrastructure:storage-system:ceph",
			direction: "outgoing",
		},
	]);
});
...
test("write wiki agent artifacts emits compact files without normalized text", () => {
	...
	try {
		const subjectNodePath = join(outputDir, "nodes", "subject:infrastructure:storage-system:ceph.json");
		const documentNodePath = join(outputDir, "nodes", "doc:ceph-osd.json");
		const pathBasedDocumentNodePath = join(outputDir, "nodes", "doc:docs/infrastructure/storage/ceph/monitoring.mdx.json");
		const result = writeWikiAgentArtifacts(outputDir, {
			documents: [
				...
				{
					id: "doc:docs/infrastructure/storage/ceph/monitoring.mdx",
					type: "document",
					title: "Ceph Monitoring",
					source_path: "docs/infrastructure/storage/ceph/monitoring.mdx",
					snippet: "Ceph monitoring guide",
		...
		assert.equal(queryIndex.documents[1].url, "/docs/infrastructure/storage/ceph/monitoring");
		...
		assert.equal(sourcePathDocumentNode.url, "/docs/infrastructure/storage/ceph/monitoring");
		...
		assert.equal(sourcePathDocumentNode.subject.url, "/docs/infrastructure/storage/ceph/monitoring");
