		...
		var e = {},
			...
			n = function (e, n) {
				...
				for (var o = e.querySelectorAll('.katex-mathml'), a = 0; a < o.length; a++) {
					...
					f &&
						(d.replaceWith ? d.replaceWith(f) : d.parentNode.replaceChild(f, d),
						(f.innerHTML = n.inline[0] + f.innerHTML + n.inline[1]));
				}
				for (var c = e.querySelectorAll('.katex-display annotation'), s = 0; s < c.length; s++) {
					...
					p.innerHTML =
						...
						p.innerHTML.substr(
							n.inline[0].length,
							p.innerHTML.length - n.inline[0].length - n.inline[1].length
						) +
						n.display[1];
				}
				return e;
				...
				if (!t.isCollapsed) {
					...
					if (l.querySelector('.katex-mathml')) {
						...
						e.clipboardData.setData('text/html', r.join('')),
							e.clipboardData.setData('text/plain', n(l).textContent),
							e.preventDefault();
					}
				}
			}),
