...
<center>
  <img src={useBaseUrl("img/nn/mlops/istio/istio-multi-primary-diff.svg")} />
</center>
...
```yaml title="network/istio/helm/istiod-values.yaml"
meshConfig:
...
helm show values istio/gateway \
    --version $ISTIO_VERSION \
    > network/istio/helm/cross-ingress-values.yaml
```

```yaml title="network/istio/helm/cross-ingress-values.yaml"
service:
  ports:
    - name: status-port
      port: 15021
...
helm upgrade cross-ingress istio/gateway \
    ...
    --version $ISTIO_VERSION \
    -n istio-system \
    -f network/istio/helm/cross-ingress-values.yaml
```
...
istioctl x create-remote-secret \
  ...
  --name=<A-cluster-name> | \
  kubectl apply -f - --context=<B-cluster-context>
```
