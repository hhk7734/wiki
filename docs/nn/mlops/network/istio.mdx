...
&& sudo mv istio-1.9.6/ /opt/
```

```shell
export PATH=/opt/istio-1.9.6/bin:$PATH
```

```shell
mkdir -p istio/istio-operator/base
```

```yaml title="istio/istio-operator/base/istio-operator.yaml"
...
```shell
istioctl install -f istio/istio-operator/base/istio-operator.yaml
```
