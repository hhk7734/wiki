...
```shell
wget https://raw.githubusercontent.com/kubernetes-sigs/aws-efs-csi-driver/release-1.3/docs/iam-policy-example.json -O iam/efs-csi-driver-policy.json
```

```python
_tags = {"Name": f"{common.cluster_name}-efs-csi-driver-policy"}
_tags.update(common.tags)
efs_csi_driver_policy = aws.iam.Policy(
    _tags["Name"],
    name=_tags["Name"],
    policy=Path("iam/efs-csi-driver-policy.json").read_text(),
    tags=_tags,
)
...
_tags = {"Name": f"{common.cluster_name}-efs-csi-driver-role"}
_tags.update(common.tags)
...
_tags = {"Name": f"{common.cluster_name}-efs-csi-driver-rpa"}
...
parameters:
  ...
  directoryPerms: "755"
  basePath: "/k8s" # Default: "/". AccessPoint의 루트 디렉토리들이 생성될 위치 입니다.
```
