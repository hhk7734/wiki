...
name = f"{const.CLUSTER_NAME}-efs-sg"
efs_sg = aws.ec2.SecurityGroup(
    ...
    opts=pulumi.ResourceOptions(parent=vpc.vpc),
    tags={
        "Name": name,
    },
)
...
aws.efs.MountTarget(
    f"{const.CLUSTER_NAME}-efs-mount-target",
...
```python
name = f"{const.CLUSTER_NAME}-efs-ng-policy"
efs_ng_policy = aws.iam.Policy(
    name,
    name=name,
    policy=json.dumps(
        {
