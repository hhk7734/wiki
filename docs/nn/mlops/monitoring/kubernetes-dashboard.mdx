...
```shell
kubectl port-forward -n monitoring svc/$(kubectl get svc -n monitoring -l app.kubernetes.io/name=kubernetes-dashboard -o jsonpath="{.items[0].metadata.name}") 8002:443
```
