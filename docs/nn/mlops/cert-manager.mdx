...
```yaml title="cert-manager/base/values.yaml"
installCRDs: true
```
...
```yaml title="letsencrypt/base/cloudflare-api-token-secret.yaml" {8}
apiVersion: v1
kind: Secret
metadata:
...
```yaml title="letsencrypt/base/letsencrypt-prod.yaml" {8,15,18}
...
```yaml title="letsencrypt/base/ingress-cert.yaml" {11-12}
apiVersion: cert-manager.io/v1
kind: Certificate
metadata:
  name: ingress-cert
...
```yaml title="letsencrypt/base/kustomization.yaml"
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization
resources:
