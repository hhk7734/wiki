...
```yaml title="cert-manager/cert-manager/base/values.yaml"
# ...
...
```shell
mkdir -p cert-manager/letsencrypt/base
```
...
```yaml title="cert-manager/letsencrypt/base/letsencrypt-prod.yaml" {8,15,18}
...
```yaml title="cert-manager/letsencrypt/base/ingress-cert.yaml" {11-12}
apiVersion: cert-manager.io/v1
kind: Certificate
metadata:
  name: ingress-cert
...
```yaml title="cert-manager/letsencrypt/base/kustomization.yaml"
...
```shell
kubectl apply -k cert-manager/letsencrypt/base
```
