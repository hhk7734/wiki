...
```python
name = f"{const.CLUSTER_NAME}-eks-karpenter-controller-policy"
...
name = f"{const.CLUSTER_NAME}-eks-karpenter-controller-role"
karpenter_controller_role = aws.iam.Role(
    name,
    name=name,
    assume_role_policy=pulumi.Output.all(
        account_id=const.AWS_ACCOUNT_ID,
        oidc_url=oidc_provider.url,
    ).apply(
...
aws.iam.RolePolicyAttachment(
    f"{const.CLUSTER_NAME}-eks-karpenter-rpa",
...
pulumi.export("eks_karpenter_controller_role_arn", karpenter_controller_role.arn)
```
...
name = f"{const.CLUSTER_NAME}-eks-ng-profile"
