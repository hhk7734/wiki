...
:::info
`--without-suid신 옵션을 사용하여 설치하면 setuid 권한이 처음부터 없는 대신 `/opt/apptainer/$APPTAINER_VERSION` 폴더 전체를 다른 곳으로 옮기더라도 실행에 문제가 없습니다.`libexec/apptainer/bin/starter-suid`가 setuid 관련 파일입니다.
:::
