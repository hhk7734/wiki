...
```yaml title="workflow/argo/cd/values.yaml"
server:
  extraArgs:
...
:::warning
적절한 RBAC가 적용된 유저를 생성한 경우 admin을 비활성화 하는 것이 좋습니다. `workflow/argo/cd/helm/values.yaml`의 `.server.config`(argocd-cm ConfigMap의 `.data`)에 `admin.enabled: "false"`를 추가하면 됩니다.
:::
