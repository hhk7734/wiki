...
<center>
  <img src={useBaseUrl('img/nn/basics/rnn/nn-rnn-gru.jpg')} />
</center>
...
\widetilde{h_t} &= \tanh(W_{x\tilde{h}} x_t + b_{x\tilde{h}} + r_t \otimes (W_{h\tilde{h}} h_{t-1} + b_{h\tilde{h}})) \\
h_t &= z_t \otimes h_{t-1} + (1 - z_t) \otimes \widetilde{h_t}\\
\end{aligned}
$$
