---
id: install
title: "OFED 설치 가이드"
sidebar_label: "설치 가이드"
description: "OFED 설치 가이드"
keywords:
  - ofed
ontology:
  role: operation
  domain: mlops
  class: networking-stack
  instance: ofed
  aspect: install
---

import Tabs from "@theme/Tabs";
import TabItem from "@theme/TabItem";

:::info
[OFED 개요](/docs/entity/mlops/networking-stack/ofed/ofed.mdx) 문서에서 스택 구성을 먼저 확인할 수 있습니다.
:::

## 설치

<Tabs
    defaultValue="mellanox"
    values={[
        {label: 'Mellanox', value: 'mellanox',},
        {label: 'DOCA', value: 'doca',}
    ]}
>

<TabItem value="mellanox">

:::danger

MLNX_OFED는 2024년 10월 LTS 이후 지원이 종료됩니다. DOCA Profile의 DOCA-OFED를 사용하시길 권장드립니다.

:::

:::info[References]

- [NVIDIA DOCS HUB / Adapter Software / NLNX_OFED / Installing the Driver](https://docs.nvidia.com/networking/display/mlnxofedv24100700/installing+the+driver)

:::

<Tabs
    groupId="os"
    defaultValue="debian"
    values={[
        {label: 'Debian', value: 'debian',}
    ]}
>

<TabItem value="debian">

```shell
sudo apt-get install -y perl pkg-config
```

</TabItem>

</Tabs>

[NVIDIA Networking / Adapter IB/VPI SW / Linux Drivers](https://network.nvidia.com/products/infiniband-drivers/linux/mlnx_ofed/)에서 설치 파일을 다운로드 받고, 압축을 해제합니다.

```shell
sudo su
```

```shell
./mlnxofedinstall <flags>
```

- `-k|--kernel <kernel>`: `$(uname -r)`가 기본값으로 설정됩니다.
- `-s|--kernel-source <kernelSource>`: `/lib/modules/$(uname -r)/build` 디렉토리가 기본값으로 설정됩니다.
- 펌웨어
  - `--without-fw-update`
- 패키지 선택
  - `--all`
  - `--basic`
- 추가 패키지 필터
  - `--kernel-only`
  - `--user-space-only`: container에서 관련 파일을 설치할 때 유용합니다.
  - `--without-<package>`
  - `--with-<package>`
    - openmpi
    - ucx
    - perftest
      - ib_write_bw
    - infiniband-diags
      - ibstat
      - ibnetdiscover
- 기타
  - `-p|--print-available`

```shell
/etc/init.d/openibd restart
```

```shell
ibstat
```

:::info

network interface 이름을 변경하고싶은 경우 [udev 규칙](/docs/operation/platform/kernel/udev/udev.mdx)을 작성합니다.

```shell
ibdev2netdev -v
```

```shell title="/etc/udev/rules.d/10-ib0.rules"
SUBSYSTEMS=="pci", KERNELS=="0000:<bus>:<device>.<function>", DRIVERS=="mlx5_core", NAME="ib0"
```

```shell
udevadm trigger --action=add
```

:::

:::tip

Kubernetes를 위한 최소 설치 예시입니다.

Host OS

```shell
./mlnxofedinstall \
    --without-fw-update \
    --kernel-only \
    --without-iser-dkms \
    --without-isert-dkms \
    --without-srp-dkms \
    --without-kernel-mft-dkms
```

container

```shell
FROM ubuntu:24.04

RUN apt-get update \
    && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
    wget \
    ca-certificates \
    perl \
    pkg-config \
    && rm -rf /var/lib/apt/lists/*

RUN wget -q https://content.mellanox.com/ofed/MLNX_OFED-24.10-3.2.5.0/MLNX_OFED_LINUX-24.10-3.2.5.0-ubuntu24.04-x86_64.tgz \
    && tar -xzf MLNX_OFED_LINUX-24.10-3.2.5.0-ubuntu24.04-x86_64.tgz \
    && cd MLNX_OFED_LINUX-24.10-3.2.5.0-ubuntu24.04-x86_64 \
    && ./mlnxofedinstall \
    --force \
    --without-fw-update \
    --user-space-only \
    --basic \
    --with-perftest \
    && cd .. \
    && rm -rf MLNX_OFED_LINUX-24.10-3.2.5.0-ubuntu24.04-x86_64*
```

:::

</TabItem>

<TabItem value="doca">

:::info[References]

- [DOCA Docs / Installation and Setup / DOCA-Host Installation and Upgrade](https://docs.nvidia.com/doca/sdk/doca-host+installation+and+upgrade/index.html)

:::

</TabItem>

</Tabs>

## 성능 테스트

### Bandwidth

서버와 클라이언트에서 아래 명령어를 각각 실행합니다.

```shell
ib_write_bw -F --report_gbits -a
```

```shell
ib_write_bw -F --report_gbits -a <ip>
```

### Latency

서버와 클라이언트에서 아래 명령어를 각각 실행합니다.

```shell
ib_write_lat -F --report_gbits -a
```

```shell
ib_write_lat -F --report_gbits -a <ip>
```

## 삭제

:::info[References]

- [NVIDIA DOCS HUB / Adapter Software / NLNX_OFED / Uninstalling the Driver](https://docs.nvidia.com/networking/display/mlnxofedv24100700/uninstalling+the+driver)

:::

```shell
/usr/sbin/ofed_uninstall.sh
```
