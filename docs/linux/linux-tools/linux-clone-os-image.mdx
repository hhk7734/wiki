---
id: linux-clone-os-image
title: Clone OS image
description: EClone OS image from disk
keywords:
  - Linux
  - clone
  - backup
...
```shell
sudo dd if=/dev/sdb of=clone.img bs=512 count=$((31115263 + 1)) status=progress
```
...
```shell
xz -dvk -T$(($(nproc) - 2)) clone.img.xz
```

```shell
sudo dd if=clone.img of=/dev/sdb bs=512 status=progress
```
