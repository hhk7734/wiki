...
DTS files are usually located in `linux/arch/<arch>/boot/dts`.

After editing DTS, execute the below commands.
...
Then, DTB will be created in `linux/arch/<arch>/boot/`. If the above command fails, check `arch/<arch>/configs`.
