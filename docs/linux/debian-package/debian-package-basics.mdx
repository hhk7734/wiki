...
아래 명령어를 통해 `<package>`와 동일한 디렉토리에 `<package>-<version>`, `<package>-<version>.tar.gz`, `<package>_<version>.orig.tar.gz`이 생성됩니다.

생성된 template files은 `<package>-<version>` 디렉토리에서 확인 가능합니다.

`<package>`디렉토리로 이동 후 아래 명령어를 실행합니다.
