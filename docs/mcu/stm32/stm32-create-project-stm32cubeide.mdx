...
STM32CubeIDE 왼쪽에 **Project Explorer**가 있는데 거기서 `<project\>.ioc` 파일을 클릭하여 열면 Perspective가 STM32CubeMX로 변경되면서 설정 창이 열립니다.

Pinout & Configuration 에서 오실레이터관련 설정 후 Clock Configuration을 설정하고 다시 Pinout & Configuration 관련 설정을 진행하는 것이 좋습니다. 클럭이 다른 설정에 영향을 줄 수 있기 때문입니다.

**STM32F103C8T6(blue-pill) 보드**를 사용하여 클럭은 최대 속도, 디버깅은 Serial Wire, LED와 UART를 사용한다고 가정하고 설정을 진행해 보겠습니다.
