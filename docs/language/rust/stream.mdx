...
`StreamExt` also provides adapters such as `map`, `filter`, `filter_map`, `inspect`, `take`, `collect`, and `for_each`.

## Inspect stream items

Use `StreamExt::inspect` to run synchronous side effects for each item without changing the stream item. The closure receives `&Self::Item`, so it is useful for logging, metrics, and debugging values as they pass through a stream pipeline.
...
`inspect` does not await inside the closure. Use `then`, `for_each`, or `for_each_concurrent` when the side effect itself is asynchronous.
