...
```shell
sudo apt-get update \
&& sudo apt install -y git gcc build-essential make sudo lsb-release \
    subversion texinfo bison flex cmake ninja-build python3 libgmp-dev \
...
&& sudo apt install -y build-essential libgtk-3-dev libegl-mesa0 libdrm-dev \
    libgbm-dev libgl1-mesa-dev libgles2-mesa-dev libglvnd-dev pkg-config \
    symlinks git vim \
&& sudo apt autoremove -y
```
