...
&& sudo apt install -y bison build-essential cmake curl flex git gcc \
    lsb-release libgmp-dev make ninja-build python python3 subversion \
    texinfo wget \
...
```shell
sudo apt update \
&& sudo apt install -y build-essential libgtk-3-dev libegl-mesa0 libdrm-dev \
    libgbm-dev libgl1-mesa-dev libgles2-mesa-dev libglvnd-dev pkg-config rsync \
    symlinks git vim \
&& sudo apt autoremove -y
```
...
```shell
sudo rsync -rlpv -e 'ssh -p <host port>' /lib /usr /etc \
...
```shell
git clone --branch=llvmorg-11.0.0 --depth=1 https://github.com/llvm/llvm-project.git $HOME/flutter-engine/llvm-project
```
...
```shell
rsync flutter_embedder.h libflutter_engine.so libflutter_linux_gtk.so icudtl.dat \
