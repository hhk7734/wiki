...
여기서 `doWork()`가 정해진 일을 처리하는데 오랜 시간이 걸린다면 메인 이벤트 루프가 블록킹 당하게 되면서 화면 갱신, 타이머, 네트워크 통신 등에 문제를 발생시킵니다. 이 상태가 길어지면 대부분 윈도우 매니저들은 응용프로그램이 응답하지 않음을 사용자에게 알려주게 됩니다.
...
<center>
  <img
    src={useBaseUrl(
      "img/lang/python/libraries/pyside2/pyside2-qthread-loop-diagram.png"
    )}
  />
</center>
