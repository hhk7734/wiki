...
class JsonFormatter(Formatter):
    ...
    def __init__(self, time_format: str = "seconds", indent: int | None = None):
        super().__init__()
...
handler.setFormatter(
    JsonFormatter(
        time_format="iso8601" if config.debug else "seconds",
        indent=2 if config.debug else None,
    )
)
logging.basicConfig(
    handlers=[handler], level=logging.DEBUG if config.debug else logging.INFO
)
```
