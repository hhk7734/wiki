...
위 명령어를 실행하면 해당 레포지토리의 `.git/hooks`에 pre-commit을 설치합니다.
...
커스텀 훅을 만들려면 `.pre-commit-hooks.yaml` 파일을 root에 정의한 레포지토리를 만들면 됩니다.
...
- id: <id> # .pre-commit-config.yaml에서 사용할 id 입니다
  ...
  entry: <relativePath> # 레포지토리의 root 기준으로 상대 경로를 적으면 됩니다
  language: script
```
...
정의된 훅을 사용하려면 `.pre-commit-config.yaml` 파일을 레포지토리의 root에 생성하면 됩니다.
