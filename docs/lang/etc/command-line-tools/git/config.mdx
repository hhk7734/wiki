...
- `git config --global diff.algorithm histogram`: diff 알고리즘을 histogram으로 변경합니다.
- `git config --global push.autoSetupRemote true`: `git push` 사용 시, 원격지 정보를 자동으로 설정합니다.
- `git config --global push.default current`: `git push` 사용 시 브랜치를 현재 브랜치로 설정합니다.
- `git config --global status.submoduleSummary true`: submodule을 사용할 때, `git status`에서 submodule 변경사항을 요약해서 보여줍니다.
