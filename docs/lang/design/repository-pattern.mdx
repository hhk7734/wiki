...
하지만 위와같이 수정하게 되면 도메인 객체의 DB에 대한 의존성이 사라지기 때문에 DB에 의한 도메인 객체 수정을 없앨 수 있습니다. 또한 서비스가 인터페이스를 의존하게 되면서 DB 레포지토리 대신 인메모리 레포지토리로 변경하여 테스트를 좀 더 수월하게 할 수 있습니다.

추가적으로 DB 자체를 변경하고 싶을 때도, 쉽게 변경할 수 있습니다.

레포지토리 인터페이스를 만들 때, 아래와 같은 사항을 따르는 것이 좋습니다.

- 데이터 저장소에 데이터를 삽입하거나 제거하는 연산을 캡슐화하는 메서드를 만든다.
- 특정 기준으로 선택하거나, 속성 값에 대한 기준으로 객체들을 조회하는 방법을 캡슐화하는 메서드를 만든다.
- 실질적으로 직접 접근해야하는 애그리거트 루트에 대해서만 레포지토리를 제공한다.
- `findOrCreate`같은 팩토리와 레포지토리의 결합된 형태의 메서드를 만드는 것은 피하는 것이 좋다.
...
`Data Mapper` 패턴은 도메인 모델과 스키마 사이의 `Mapper`를 사용자가 정의하고, ORM의 데이터베이스 기술 추상화만 이용하는 것입니다. Mapper와 행위에 대한 구현을 Repository로 구현해서 의존성을 역전시킬 수 있습니다.
...
여기에 레포지토리 패턴을 추가하면 패턴 자체의 장점과 ORM을 사용하므로써 얻는 장점 모두를 가질 수 있습니다.
