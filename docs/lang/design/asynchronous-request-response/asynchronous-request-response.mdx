---
id: asynchronous-request-response
title: Asynchronous Request-Response
sidebar_label: Async Request-Response
description: Asynchronous Request-Response
keywords:
  - async
  - long-running
---

## 장기 실행 작업(Long Running Task)

클라이언트에서 서버에 요청을 보냈을 때, 서버에서 신속하게 응답을 보내지 못하지만 클라이언트는 서버의 응답을 받아야하는 경우가 있습니다. 예를 들어 처리하는 데 1초가 걸리는 CPU 바운드 작업을 요청했을 때, 대량의 요청이 오는 경우 서버는 1초 이상 응답을 못하거나 요청 자체를 처리하지 못할 수 있습니다.

이런 경우 고민해야하는 사항은

- 서버가 요청을 처리하는 방법
- 서버가 처리를 완료한 후 클라이언트에게 응답을 보내는 방법
- 완료되기 전에 클라이언트가 요청을 취소하는 방법

입니다.

이러한 상황에 대해 Scale up/out만으로 처리할 수 없는 경우 클라이언트, 서버, 워커 사이에서 요청(Command), 상태(Status), 응답(Response)에 대한 설계로 해결할 수 있습니다.

## 요청(Command)

```mermaid
sequenceDiagram
  participant Client
  participant Server
  participant Queue
  participant Worker

  Client->>Server: POST Command
  Server->>Queue: Command
  Server-->>Client: 202 Accepted
  Queue-->>Worker: Command
```

## 상태(Status) && 응답(Result)

```mermaid
sequenceDiagram
  participant Client
  participant Server
  participant Queue
  participant Worker

  alt
    Worker->>Server: Status
  else
    Worker->>Queue: Status
    Queue-->>Server: Status
  end
```

### Polling

Polling이 많아지면 서버에 부하가 걸리기 때문에 이를 완화하기 위해 `Retry-After: <date||seconds>` 헤더를 사용하여 클라이언트가 요청 간격을 조절할 수 있도록 하는 것이 좋습니다.

```mermaid
sequenceDiagram
  participant Client
  participant Server
  participant Queue
  participant Worker

  loop
    Client->>Server: GET Status
    Server-->>Client: Status
  end

  Client->>Server: GET Result
  Server-->>Client: Result
```

### SSE(Server Sent Event) || Websocket

```mermaid
sequenceDiagram
  participant Client
  participant Server
  participant Queue
  participant Worker

  Server->>Client: Status || Result
```

## 취소(Cancel)

```mermaid
sequenceDiagram
  participant Client
  participant Server
  participant Queue
  participant Worker

  Client->>Server: POST Cancel
  Server-->>Client: 202 Accepted

  Queue-->>Worker: Command
  Worker->>Server: GET Status
  Server-->>Worker: CancelRequested
  Worker->>Server: PUT Canceled
```

## 속도 제한

- 요청 제한(Request Limit)
  - 속도 제한(Request Rate Limit): 시간당 요청 개수 제한
  - 개수 제한(Request Count Limit): 요청 개수 제한
- 처리 제한(Processing Limit)
  - 속도 제한(Processing Rate Limit): 시간당 처리 개수 제한
  - 개수 제한(Processing Count Limit): 처리 개수 제한

요청 속도 제한은 DDoS, 헤비 유저 등에 의한 서버 과부하 방지를 목적으로 한다면 요청 개수 제한, 처리 속도 제한, 처리 개수 제한은 다수의 장기 실행 작업에 의한 서버 자원 부족 방지를 목적으로 하거나 비지니스 모델에 따른 제한을 주 목적으로 합니다.

각 제한은 전체 요청량이나 처리량에 의해 제한 될 수도 있고, IP, 유저 등 특정 조건에 의해 제한 될 수도 있습니다.

## 처리 방식

- 컨슈머가 직접처리, 컨슈머를 스케일 아웃하는 방식으로 처리량을 늘림
- 워크플로우 엔진을
