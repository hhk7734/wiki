...
Process of inference using mini-batch and $\mathbf{w}$ is called **Feed-forward**. $a^l_j$ and $z^l_j$ ​​can be obtained through this process.
...
Since $z^l_j$ was obtained from Feed-forward, If we know $\mathbf{\delta}^{l+1}$, we can get $\delta^l_j$ as below.
...
Since $a^L_j$ was obtained from Feed_forward and $\delta^L_j = (a^L_j - y_j) \, \sigma' \left( z^L_j \right)$, we can get the errors like this:
...
Set initail weights and biases to random and repeat process **Feed-forward -> Back-propagation -> weights and biases update**. When it is judged that $C$ cannot be made smaller, the final weight and bias are determined.
