...
```shell
helm pull amd-gpu/amd-gpu --version 0.13.0
```

```shell
helm show values amd-gpu/amd-gpu --version 0.13.0 > amd-gpu-0.13.0.yaml
```
...
```shell
helm template amd-gpu amd-gpu-0.13.0.tgz \
    -n kube-system \
...
```shell
helm upgrade amd-gpu amd-gpu-0.13.0.tgz \
    --install \
    --history-max 5 \
    -n kube-system \
    -f amd-gpu-values.yaml
