...
```shell
helm pull prometheus-community/kube-prometheus-stack --version 57.0.1
```

```shell
helm show values kube-prometheus-stack-57.0.1.tgz \
...
```shell
helm template prometheus-stack kube-prometheus-stack-57.0.1.tgz \
    -n monitoring \
...
```shell
helm upgrade prometheus-stack kube-prometheus-stack-57.0.1.tgz \
    --install \
    --history-max 5 \
    -n monitoring \
    -f kube-prometheus-stack-values.yaml
