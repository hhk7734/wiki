...
```shell
helm pull prometheus-community/prometheus-node-exporter --version 4.30.3
```

```shell
helm show values prometheus-community/prometheus-node-exporter \
    --version 4.30.3 \
    > prometheus-node-exporter-4.30.3.yaml
```
...
```shell
helm template node-exporter prometheus-node-exporter-4.30.3.tgz \
    -n monitoring \
...
```shell
helm upgrade node-exporter prometheus-node-exporter-4.30.3.tgz \
    --install \
    --history-max 5 \
    -n monitoring \
    -f prometheus-node-exporter-values.yaml
...
- `node` `_cpu` `_seconds_total`
  - 각 mode 별 CPU 사용 누적 시간입니다.
  - 1 초가 지났을 때 각 mode 별 CPU 사용 시간의 총 합은 1 이 증가합니다.
