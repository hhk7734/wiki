...
```mermaid
sequenceDiagram
  Note right of apiserver: User creates Deployment
  apiserver -->> DeploymentController: Deployment Create Event
  DeploymentController ->> DeploymentController: Reconcile Deployment
  DeploymentController ->> apiserver: Create ReplicaSet

  apiserver -->> ReplicaSetController: ReplicaSet Create Event
  ReplicaSetController ->> ReplicaSetController: Reconcile ReplicaSet
  ReplicaSetController ->> apiserver: Create Pod

  apiserver -->> Scheduler: Pod Create Event
  Scheduler ->> Scheduler: Reconcile Pod
  Scheduler ->> apiserver: Schedule Pod to Node

  participant Kubelet as Node(Kubelet)

  apiserver -->> Kubelet: Pod Update Event
  Kubelet ->> Kubelet: Reconcile Pod(Start Container)
  Kubelet ->> apiserver: Update Pod Status

  apiserver -->> ReplicaSetController: Pod Update Event
  ReplicaSetController ->> ReplicaSetController: Reconcile ReplicaSet
  ReplicaSetController ->> apiserver: Update ReplicaSet Status

  apiserver -->> DeploymentController: ReplicaSet Update Event
  DeploymentController ->> DeploymentController: Reconcile Deployment
  DeploymentController ->> apiserver: Update Deployment Status
```
