...
:::warning
처리가 오래걸리는 작업에 대해, 스케일 다운이 발생하면, 처리가 완료되지 않은 작업이 중단될 수 있습니다. 이 경우 [terminationGracePeriodSeconds](/docs/mlops/mlops/kubernetes/workloads/pod-lifecycle#termination)를 사용하거나, `ScaledJob`를 활용하는 방향으로 변경해야합니다.
:::
