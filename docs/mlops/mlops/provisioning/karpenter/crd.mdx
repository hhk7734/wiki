...
- https://karpenter.sh/v0.21.1/concepts/provisioners/
...
- https://karpenter.sh/v0.21.1/concepts/provisioners/#speckubeletconfiguration
...
```yaml
spec:
  # Provisioner가 관리하는 Node에 추가될 label
  labels:
    <key>: <value>
```
...
- https://karpenter.sh/v0.21.1/concepts/provisioners/#specrequirements
...
:::info
AWS의 경우 `<instanceType>`=`<instanceCategory><instanceGeneration>[<attribute>].<instanceSize>` 으로 결정됩니다. 예를 들어 `c5n.xlarge`는 `c`, `5`, `n`, `xlarge`로 해석할 수 있습니다.
:::
...
- `node.kubernetes.io/instance-type`: 원하는 유형을 직접 지정할 수 있습니다
- `karpenter.sh/capacity-type`: `on-demand`|`spot`, 둘 다 선언하면 우선순위는 `spot` > `on-demand` 입니다
...
- https://karpenter.sh/v0.21.1/concepts/node-templates/
