...
```shell
helm pull rook-release/rook-ceph --version v1.13.3
```

```shell
helm show values rook-ceph-v1.13.3.tgz > rook-ceph-v1.13.3.yaml
```
...
```shell
helm template rook-ceph-v1.13.3.tgz \
    -n rook-ceph \
...
```shell
helm upgrade rook-ceph-v1.13.3.tgz \
    --install \
    --history-max 3 \
    -n rook-ceph \
    -f rook-ceph-values.yaml
...
```shell
helm pull rook-release/rook-ceph-cluster --version v1.13.3
```

```shell
helm show values rook-ceph-cluster-v1.13.3.tgz > rook-ceph-cluster-v1.13.3.yaml
```
...
```shell
helm template rook-ceph-cluster-v1.13.3.tgz \
    -n rook-ceph \
...
```shell
helm upgrade rook-ceph-cluster-v1.13.3.tgz \
    --install \
    --history-max 3 \
    -n rook-ceph \
    -f rook-ceph-cluster-values.yaml
