...
```shell
asdf install istioctl 1.16.1
```

```shell
asdf global istioctl 1.16.1
```

```shell
asdf local istioctl 1.16.1
```
...
```shell
helm template istiod-1-16-1 istio/istiod \
    --version 1.16.1 \
    -n istio-system \
...
```shell
helm upgrade istiod-1-16-1 istio/istiod \
    --install \
    --version 1.16.1 \
    -n istio-system \
    --history-max 3 \
...
| --------- | ------------------- | ------------------------------- |
| Namespace | istio.io/rev=1-16-1 | istio-injection=disabled        |
| Pod       | istio.io/rev=1-16-1 | sidecar.istio.io/inject="false" |
