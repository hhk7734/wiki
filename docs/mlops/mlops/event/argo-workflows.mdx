...
```yaml title="workflow/argo/workflows/base/virtual-service.yaml" {8,10}
...
```yaml title="workflow/argo/cd/helm/values.yaml"
dex:
  env:
...
```yaml title="workflow/argo/workflows/helm/values.yaml"
server:
  extraArgs:
    - --auth-mode=sso
    - --auth-mode=client
