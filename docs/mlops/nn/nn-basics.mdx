...
훈련을 위한 데이터셋이 큰 경우, 시간이 많이 걸릴 수 있습니다. 전체 데이터셋에서 $m$ 만큼 데이터를 선택하여 `mini-batch`를 만들어 훈련하는 방식을 `Stochastic gradient descent`이라고 합니다.

$$
\nabla Loss = \frac{1}{n}\sum_x{\nabla Loss_x} \approx \frac{1}{m}\sum^m_{i=1}{\nabla Loss_{X_i}}
$$

전체 데이터셋에서 랜덤으로 데이터를 선택해 만든 데이터셋 $X_1, X_2, ..., X_m$ 을 `mini-batch`라고 합니다.
